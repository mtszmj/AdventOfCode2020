...
namespace AdventOfCode2020
{
    public class Day18Task2
    {
        ...
        public long SolveOne(string input)
        {
            ...
            while(list.Any(x => x is Add))
            {
                for (var i = 0; i < list.Count; i++)
                {
                    if (i < list.Count - 2 && list[i] is Value left && list[i + 1] is Add add && list[i + 2] is Value right)
                    {
                        newList.Add(new Value { Val = add.Perform(left.Val, right.Val) });
                        i += 2;
                    }
                    else
                    {
